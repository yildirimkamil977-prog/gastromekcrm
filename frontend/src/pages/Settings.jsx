...
export default function Settings() {
  ...
  if (loading || !form) return <div className="p-8 text-slate-400">{t("common.loading")}</div>;
      ...
      <Tabs defaultValue="company">
        <div className="overflow-x-auto -mx-1 px-1">
          <TabsList className="w-max">
            <TabsTrigger value="company" data-testid="tab-company">{t("settings.tabCompany")}</TabsTrigger>
            <TabsTrigger value="bank" data-testid="tab-bank">{t("settings.tabBank")}</TabsTrigger>
            <TabsTrigger value="social" data-testid="tab-social">{t("settings.tabSocial")}</TabsTrigger>
            <TabsTrigger value="email" data-testid="tab-email">{t("settings.tabEmail")}</TabsTrigger>
            <TabsTrigger value="quote" data-testid="tab-quote">{t("settings.tabQuote")}</TabsTrigger>
          </TabsList>
        </div>
              ...
              return (
                <div key={i} className="border border-slate-200 rounded-lg p-4 space-y-3" data-testid={`bank-slot-${i}`}>
                  <div className="flex items-center justify-between">
                    <div className="text-xs font-bold uppercase tracking-wider text-slate-500">{t("settings.bank")} {i + 1}</div>
